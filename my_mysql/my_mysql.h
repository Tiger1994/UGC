...
void SelectOneDayFromMysql(const std::string &date, const std::string str_limit, \
  ...
  fprintf(stdout, "%.*s\n", int(res_num.size()), res_num.c_str()); 
  rapidjson::StringBuffer str_buf;
  rapidjson::Writer<rapidjson::StringBuffer> writer(str_buf);
  writer.StartObject();
  writer.Key("total");
  writer.Int(res.size());
  size_t ceil = offset + limit > res.size() ? res.size() : offset + limit;
...
int WriteRecordToMysql(const rapidjson::Document &json_doc) {
  ...
  conn.connect(db, server, user, pass);
  mysqlpp::Query query = conn.query(cmd.c_str());
  bool exec_res = query.exec();
  if(exec_res) return 0;
  else return 1;
}
